...
export async function GET() {
  ...
  try {
    ...
    const enriched = players.map((p) => {
      ...
      const { tone, labelKey } = readinessStatus(score, workspace.readinessThreshold);
    ...
    return NextResponse.json({
      kpis: {
        ...
        pendingInvitations: invites.filter((i) => inviteStatus(i) === "pending").length,
        reportsToday: enriched.filter((p) => p.loggedToday).length,
      ...
      recentActivity: [
        ...recentLogs.map((log) => ({
          ...
          tone: readinessStatus(log.score, workspace.readinessThreshold).tone,
        })),
