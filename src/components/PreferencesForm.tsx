...
export default function PreferencesForm({ settings = false, onboarding = false }: { settings?: boolean; onboarding?: boolean }) {
  ...
  return <form onSubmit={save} className="flex flex-col gap-5" dir={fa ? "rtl" : "ltr"}>
    {onboarding && <div><h1 className="font-display text-3xl font-bold text-white">{t(locale, "preferences.title")}</h1><p className="mt-2 text-sm text-smoke-3">{t(locale, "preferences.subtitle")}</p></div>}
    <label className="flex flex-col gap-2"><span className="text-sm font-medium text-smoke-4">{t(locale, "preferences.language")}</span>
    ...
    <label className="flex flex-col gap-2"><span className="text-sm font-medium text-smoke-4">{t(locale, "preferences.timezone")}</span>
      <input className="input-field" value={query} onChange={(e) => setQuery(e.target.value)} placeholder={t(locale, "preferences.timezoneSearch")} aria-label={t(locale, "preferences.timezoneSearch")} />
    ...
    <button className="btn-primary" disabled={saving}>{saving ? t(locale, "preferences.saving") : (settings ? t(locale, "preferences.save") : t(locale, "preferences.continue"))}</button>
  </form>;
}
